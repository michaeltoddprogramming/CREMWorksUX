...
const Gear = () => {
  ...
  const getProductBadges = (productId: string) => {
    // Use product ID to generate consistent random badges
    const hash = productId.split('').reduce((a, b) => {
      a = ((a << 5) - a) + b.charCodeAt(0);
      return a & a;
    }, 0);
    ...
    // Each badge has roughly 30-40% chance of appearing
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="flex gap-8">
          ...
          <div className="w-64 flex-shrink-0 sticky top-4 self-start">
            <div className="filter-section max-h-[calc(100vh-2rem)] overflow-y-auto bg-white p-6 rounded-lg shadow-sm border">
                  ...
                  {[
                    ...
                    <div key={range.label} className="flex items-center">
                      <input
                        ...
                        className="w-4 h-4 text-blue-600"
                      />
                      <label htmlFor={range.label} className="ml-2 text-sm text-gray-700 cursor-pointer">
                        {range.label}
                      </label>
                    </div>
                  ))}
                  ...
                  <div key={product._id} className="product-card group cursor-pointer flex" onClick={() => navigate(`/gear/${product._id}`)}>
                    <div className="relative w-64 h-48 flex-shrink-0">
                      ...
                      <div className="absolute top-3 left-3 flex flex-wrap gap-1">
                        ...
                        <span className="badge-rods">{product.category}</span>
                      </div>
                    </div>
